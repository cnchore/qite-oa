import { query,queryById,save,deleteById} from '../services/seal'
...
export default {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      let _payload=parse(location.hash.split('#/seal?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ seal }) => seal.currentItem.id)
