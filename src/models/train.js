import { query,queryById,save,deleteById} from '../services/train'
...
export default {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      let _payload=parse(location.hash.split('#/train?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ train }) => train.currentItem.id)
