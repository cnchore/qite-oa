import { query,queryById,save,deleteById} from '../services/renoSubsidy'
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname==='/renoSubsidy') {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      let _payload=parse(location.hash.split('#/renoSubsidy?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ renoSubsidy }) => renoSubsidy.currentItem.id)
