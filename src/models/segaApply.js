import { query,queryById,save,deleteById} from '../services/segaApply'
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname==='/segaApply') {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      let _payload=parse(location.hash.split('#/segaApply?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ segaApply }) => segaApply.currentItem.id)
