...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        ...
        if (location.pathname === '/travelReimburse') {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      let _payload=parse(location.hash.split('#/travelReimburse?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ travelReimburse }) => travelReimburse.currentItem.id)
