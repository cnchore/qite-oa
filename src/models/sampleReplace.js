import { query,queryById,save,deleteById} from '../services/sampleReplace'
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname==='/sampleReplace') {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      payload=parse(location.hash.split('#/sampleReplace?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ sampleReplace }) => sampleReplace.currentItem.id)
