import { query,queryById,save,deleteById} from '../services/materialSupport'
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname==='/materialSupport') {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      let _payload=parse(location.hash.split('#/materialSupport?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ materialSupport }) => materialSupport.currentItem.id)
