...
import { getTaskInfo,getDiagramByBusiness,getDic } from '../../services/workFlow'

export default {
  namespace: 'cashierPaymentDetail',
  state: {
    data: {},
    employeeList:[],
    dicList:[],
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
        const match = pathToRegexp('/cashierPayment/:id').exec(location.pathname)
        if (match) {
