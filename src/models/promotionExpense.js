import { query,queryById,save,deleteById} from '../services/promotionExpense'
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname==='/promotionExpense') {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      let _payload=parse(location.hash.split('#/promotionExpense?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ promotionExpense }) => promotionExpense.currentItem.id)
  ...
  reducers: {
    ...
    setScheduleEditable(state,action){
      return {...state,isScheduleEditable:action.payload}
    },
  },
