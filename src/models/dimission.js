import { query,queryById,save,deleteById,submit,queryEmployee,getDic } from '../services/dimission'
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        ...
        if (location.pathname === '/dimission') {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      payload=parse(location.hash.split('#/dimission?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ dimission }) => dimission.currentItem.id)
