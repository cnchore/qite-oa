import { query,queryById,save,deleteById} from '../services/shopUpgrade'
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname==='/shopUpgrade') {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      let _payload=parse(location.hash.split('#/shopUpgrade?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ shopUpgrade }) => shopUpgrade.currentItem.id)
