import { query,queryById,save,deleteById} from '../services/sampleRoom'
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname==='/sampleRoom') {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      let _payload=parse(location.hash.split('#/sampleRoom?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ sampleRoom }) => sampleRoom.currentItem.id)
