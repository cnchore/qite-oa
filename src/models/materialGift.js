import { query,queryById,save,deleteById} from '../services/materialGift'
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname==='/materialGift') {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      let _payload=parse(location.hash.split('#/materialGift?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ materialGift }) => materialGift.currentItem.id)
