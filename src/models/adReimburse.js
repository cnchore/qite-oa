...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        if (location.pathname==='/adReimburse') {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      let _payload=parse(location.hash.split('#/adReimburse?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ adReimburse }) => adReimburse.currentItem.id)
