...
import { getMyTaskDonePage,getTaskInfo,audit,getDic,getOrg } from '../services/workFlow'
...
export default {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      payload=parse(location.hash.split('#/complete?')[1]); 
      // payload = parse(location.search.substr(1))
      const userInfo = JSON.parse(sessionStorage.getItem(`${prefix}userInfo`));
      
      if (userInfo && userInfo.data) {
        payload={...payload,rows:payload.pageSize}
        const data = yield call(getMyTaskDonePage, payload)
