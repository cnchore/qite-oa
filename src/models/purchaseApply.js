import { query,queryById,save,deleteById,submit,queryEmployee,getDic } from '../services/purchaseApply'
...
export default {
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen(location => {
        ...
        if (location.pathname === '/purchaseApply') {
  ...
  effects: {
    *query ({ payload }, { call, put }) {
      ...
      payload=parse(location.hash.split('#/purchaseApply?')[1]); 
    ...
    *update ({ payload }, { select, call, put }) {
      const id = yield select(({ purchaseApply }) => purchaseApply.currentItem.id)
