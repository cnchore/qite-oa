...
const Detail = ({ shopDetail }) => {
  const { data,employeeList,commentList,taskNode } = shopDetail
  ...
  return (
    <div className={cs({'content-inner':true,...JSON.parse(`{"audited${data && data.state && data.state}":true}`) })}>
      ...
      <ShopDetailPage data={data} employeeList={employeeList} />
      {
...
export default connect(({ shopDetail, loading }) => ({ shopDetail, loading: loading.models.shopDetail }))(Detail)
