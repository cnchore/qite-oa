...
const Detail = ({ urgentOrderDetail }) => {
  const { data,employeeList,commentList,taskNode,dicList } = urgentOrderDetail
  setPrintData(data,employeeList,null,commentList,dicList)
  return (
    <div className={cs({'content-inner':true,...JSON.parse(`{"audited${data && data.state && data.state}":true}`) })}>
      ...
      <UrgentOrderDetailPage data={data} employeeList={employeeList}  dicList={dicList}/>
      {
...
Detail.propTypes = {
  urgentOrderDetail: PropTypes.object,
  loading: PropTypes.bool,
}

export default connect(({ urgentOrderDetail, loading }) => ({ urgentOrderDetail, loading: loading.models.urgentOrderDetail }))(Detail)
