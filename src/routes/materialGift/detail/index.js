...
const Detail = ({ materialGiftDetail }) => {
  const { data,employeeList,commentList,taskNode } = materialGiftDetail
  ...
  return (
    <div className={cs({'content-inner':true,...JSON.parse(`{"audited${data && data.state && data.state}":true}`) })}>
      ...
      <MaterialGiftDetailPage data={data} employeeList={employeeList} />
      {
...
Detail.propTypes = {
  materialGiftDetail: PropTypes.object,
  loading: PropTypes.bool,
}

export default connect(({ materialGiftDetail, loading }) => ({ materialGiftDetail, loading: loading.models.materialGiftDetail }))(Detail)
