...
const Detail = ({ materialSupportDetail }) => {
  const { data,employeeList,commentList,taskNode } = materialSupportDetail
  ...
  return (
    <div className={cs({'content-inner':true,...JSON.parse(`{"audited${data && data.state && data.state}":true}`) })}>
      ...
      <MaterialSupportDetailPage data={data} employeeList={employeeList} />
      {
...
Detail.propTypes = {
  materialSupportDetail: PropTypes.object,
  loading: PropTypes.bool,
}

export default connect(({ materialSupportDetail, loading }) => ({ materialSupportDetail, loading: loading.models.materialSupportDetail }))(Detail)
