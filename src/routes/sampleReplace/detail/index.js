...
const Detail = ({ sampleReplaceDetail }) => {
  const { data,employeeList,commentList,taskNode } = sampleReplaceDetail
  ...
  return (
    <div className={cs({'content-inner':true,...JSON.parse(`{"audited${data && data.state && data.state}":true}`) })}>
      ...
      <SampleReplaceDetailPage data={data} employeeList={employeeList} />
      {
...
Detail.propTypes = {
  sampleReplaceDetail: PropTypes.object,
  loading: PropTypes.bool,
}

export default connect(({ sampleReplaceDetail, loading }) => ({ sampleReplaceDetail, loading: loading.models.sampleReplaceDetail }))(Detail)
