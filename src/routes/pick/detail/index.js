...
const Detail = ({ pickDetail }) => {
  const { data,employeeList,commentList,taskNode } = pickDetail
  ...
  return (
    <div className={cs({'content-inner':true,...JSON.parse(`{"audited${data && data.state && data.state}":true}`) })}>
      ...
      <PickDetailPage data={data} employeeList={employeeList} />
      {
...
Detail.propTypes = {
  pickDetail: PropTypes.object,
  loading: PropTypes.bool,
}

export default connect(({ pickDetail, loading }) => ({ pickDetail, loading: loading.models.pickDetail }))(Detail)
