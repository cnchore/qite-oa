...
const Detail = ({ cardDetail }) => {
  const { data,employeeList,commentList,taskNode } = cardDetail
  ...
  return (
    <div className={cs({'content-inner':true,...JSON.parse(`{"audited${data && data.state && data.state}":true}`) })}>
      ...
      <CardDetailPage data={data} employeeList={employeeList} />
      {
...
export default connect(({ cardDetail, loading }) => ({ cardDetail, loading: loading.models.cardDetail }))(Detail)
