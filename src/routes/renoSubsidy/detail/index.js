...
const Detail = ({ renoSubsidyDetail }) => {
  const { data,employeeList,commentList,taskNode } = renoSubsidyDetail
  ...
  return (
    <div className={cs({'content-inner':true,...JSON.parse(`{"audited${data && data.state && data.state}":true}`) })}>
      ...
      <RenoSubsidyDetailPage data={data} employeeList={employeeList} />
      {
...
Detail.propTypes = {
  renoSubsidyDetail: PropTypes.object,
  loading: PropTypes.bool,
}

export default connect(({ renoSubsidyDetail, loading }) => ({ renoSubsidyDetail, loading: loading.models.renoSubsidyDetail }))(Detail)
