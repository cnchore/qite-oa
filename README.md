...
### 未完事宜
1. 图标更换；
2. webpack打包优化：antd分离，热发布;

### 2017-8-21

1. 知识库－新增页面，添加发布功能；
2. 申购申请－申购类型取消默认值；
3. 修改表单打印申请日期显示错误等bug；
4. 优化打包，实现一次打包，正式和测试环境均可使用；
5. 所有业务提交不成功，后台返回信息显示bug修改；
6. 请假申请－业务状态4，值修改为‘审批通过并已销假’，业务状态－2，值修改为‘待销假’;
...
2. 申购申请－添加申购类型；
3. 采购申请－数量可以修改，对接后台；
4. 申请人消息提醒－采购申请、(出差)费用报销、付款申请、合同审批、申购申请、预算申请，流程办结后，添加消息内容“请打印表单，并黏贴附件交财务部”；
5. 业务审批－选择‘退回上一步、退回申请人修改‘时，审批意见框默认为’不同意，退回修改‘；
6. 我的申请－添加外勤申请；
...
### 2017-8-14
1. 申请表单－修改数量按整数加减；
2. 采购申请－来自采购单的数量，修改为允许修改；
3. 附件－采购申请、费用报销、付款申请、合同审批、申购申请，修改为必传；
4. 审批－点击同意／同意并办结，审批意见添加自动输入‘同意’;
5. 流程结束－添加结束标志；
6. 表单打印－添加申购申请表单；

### 2017-8-10

1. 流程，返回上一步，添加：可选择审批人；
2. 采购申请，金额小数点位数bug，修改为只取2位；
3. 登录用户，显示真实名称，不显示账号；
4. 所有申请页面，申请单号、申请时间，无值时，显示为‘系统自动生成’
